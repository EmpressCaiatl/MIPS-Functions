...
#				 lists. Store the results and find basic stats.
...
#    Arguments:
#	$a0	- starting address of the aSides array		a
#	$a1	- starting address of the bSides array		b
#	$a2	- starting address of the cSides array		c
#	$a3	- starting address of the dSides array		d
#	($fp)	- length								e
#	4($fp)	- starting address of the diags array	f

#    Returns:
#	diag[] areas array via passed address ($v0)
...
calcDiagonals:
	...
	lw  $s2, ($fp)		# load in length value into s2
	lw  $s1, 4($fp)		# load value of addr of diags array
	
	move $s0, $a0

	calcLoop:
		lw $t2, ($s0)			# get aSides[i]
		lw $t3, ($a1)			# get bSides[i]
		lw $t4, ($a2)			# get cSides[i]
		lw $t5, ($a3)			# get dSides[i]

		# CALCULATION
		mul $t6, $t3, $t3		# b[i]^2
		mul $t6, $t6, $t2		# a[i] * b[i]^2
 
		mul $t7, $t2, $t2		# a[i]^2
		mul $t7, $t7, $t3		# a[i]^2 * b[i]  

		mul $t4, $t4, $t4		# c[i]^2
		mul $t8, $t2, $t4		# a[i] * c[i]^2

		mul $t5, $t5, $t5		# d[i]^2
		mul $t9, $t3, $t5		# b[i] * d[i]^2

		sub $t6, $t6, $t7		# (a[i] * b[i]^2) - (a[i]^2 * b[i])
		sub $t6, $t6, $t8		# (...) - a[i] * c[i]^2
		add	$t6, $t6, $t9		# (...) + b[i] * d[i]^2

		li $t7, 0				# clear

		sub $t7, $t3, $t2		# b[i] - a[i]
		div $t6, $t6, $t7		# (...) / b[i] - a[i]

		move $a0, $t6			# set up argument for iSqrt
		jal iSqrt				# call iSqrt() function

		sw $v0, ($s1)			# store @ diags[i]

		# increment our addrs by word-size for next iteration
		add $s1, $s1, 4
		add $s0, $s0, 4
		add $a1, $a1, 4
		add $a2, $a2, 4
	...
	# set args for gnomeSort
	lw $a0, 4($fp)
	lw $t0, ($fp)
	move $a1, $t0
	jal gnomeSort

	lw  $v0, 4($fp)				# load value of addr of diags array
...
gnomeSort:
	...
	# set i and j
	li $t0, 4				# i = 1 (*4 working with word-size)
	li $t1, 8				# j = 2 (*4)
...
.ent	findSum
findSum:
	li $v0, 0				# clear register
	move $s0, $a0			# diags[i]
	move $s1, $a1			# grab length
	li $t4, 0				# counter
	sumLoop:
		beq $t4, $s1, endLoop	
		lw $t1, ($s0)		# grab value from array
		add $v0, $v0, $t1	# running sum
		# increments
		add $s0, $s0, 4
		add $t4, $t4, 1
	j sumLoop
...
findAverage:
	...
	move $s0, $a0		# get value of addr of diags array

	jal findSum			# get sum for average calculation
	mtc1 $v0, $f0		# move sum into float register

	cvt.s.w $f0, $f0	# convert sum into float

	mtc1 $a1, $f1		# move len into float register
	cvt.s.w $f1, $f1	# convert len into float

	div.s $f0, $f0, $f1	# average = sum / len
...
diagonalsStats:
	...
	lw $t2, ($s0)			# grabs diags[0] 
	sw $t2, ($a2)			# mins = diags[0]
	
	move $t1, $a1			# grabs length
	mul $t1, $t1, 4
	subu $t1, $t1, 4		# len = len - 1
	addu $s0, $s0, $t1		
	lw $t3, ($s0)			# grabs diags[len-1]
	sw $t3, ($a3)			# max = diags[len-1]
	# ------------------------------------------
	# MEDIAN -----------------------------------
	# first test if odd or even
	move $t0, $a1
	li $t1, 2
	div $t0, $t1	

	mfhi $t1			# remainder in register hi

	# if remainder is 0, len is even, else odd
	beq $t1, 0, isEven
	...
	isOdd:
		move $t0, $a1			# get len
		li $t1, 2			
		div $t2, $t0, $t1		# get index
		mul $t2, $t2, 4

		move $s3, $a0
		addu $s3, $s3, $t2		# get to address @ index
		lw $t4, ($s3)			# gets value @ index
		sw $t4, ($s4)			# store median
		j nextNum
	isEven:
		move $t0, $a1			# get len
		li $t1, 2
		div $t2, $t0, $t1		# get first index
		mul $t2, $t2, 4
		...
		addu $s3, $s3, $t2		# get to address @ index
		lw $t4, ($s3)			# gets value @ index
		addu $s3, $s3, 4		# get address @ next index
		lw $t5, ($s3)			# get value @ next index

		addu $t6, $t4, $t5		# add both indexes 
		div $t6, $t6, $t1		# (index1+index2)/2

		sw $t6, ($s4)			# store median
	# ------------------------------------------
	nextNum:
	jal findAverage
	s.s $f0, ($s5)
